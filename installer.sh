...
LOGFILE="$HOME/niri_setup.log"

# Default dotfiles dir if not set in paths.conf
...
for cfg in modules.conf paths.conf packages.conf logging.conf user.conf; do
...
for lib in colors.sh log.sh utils.sh checks.sh prompt.sh; do
	source "$LIB_DIR/$lib"
done
...
MODULES_ORDER=(dotfiles packages paru zsh starship themes fonts symlinks flatpak cleanup)
